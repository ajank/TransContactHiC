...
def parse_arguments(args=None):
    ...
    parser = argparse.ArgumentParser(prog='set_bam_flag', description='Set the given flag(s) for all alignments in a BAM file.')
    parser.add_argument('flag', type=int, help='integer SAM flag(s) to be set')
    ...
    with pysam.AlignmentFile(args.bam_in, 'rb') as bam_in:
        ...
        with pysam.AlignmentFile(args.bam_out, 'wb', header=bam_header) as bam_out:
            for line in bam_in.fetch(until_eof=True):
                # write each alignment line with the given flag(s) set
                line.flag |= args.flag
                bam_out.write(line)
